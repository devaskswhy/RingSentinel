...
const CALIBRATION: Explainer = {
  ...
  text: {
    en: `The threshold is 0.3, and it was measured rather than chosen. Sweeping it across the tuning split, anything between 0.25 and 0.35 found all 8 rings with zero false flags. Below that band a false flag appears. Above 0.4, real rings start being missed. 0.3 sits in the centre of the flat region, and that is the whole point. The weakest cluster scores 0.37, so the threshold could move seven hundredths and nothing would change. But that plateau is a property of this corpus, not of the world. Run against 524,834 real transactions from the IEEE-CIS dataset, the same threshold admitted almost every candidate it produced, because real payment data compresses the score range. The ranking survives; the constant does not. On unfamiliar data the honest rule is a review budget, not a number.`,
...
/**
 * The real-data result, spoken.
 *
 * This is the only explainer on the site whose subject is a FAILURE, and it is
 * the one most worth listening to. Every figure is read from the run reported
 * under "On real data we did not create" in the README: 590,540 rows, 524,834
 * usable, a 2.46% base rate, 1.04x at the calibrated threshold, 1.12x in the
 * top tenth, 0.36x in the top 2%. The superseded 1.62x is named out loud
 * rather than quietly dropped, because the correction is the part that makes
 * everything else on this page credible.
 */
const REAL_DATA: Explainer = {
  id: "real-data",
  question: {
    en: "Does it work on real data?",
    hi: "क्या यह असली डेटा पर काम करता है?",
    es: "¿Funciona con datos reales?",
    fr: "Fonctionne-t-il sur des données réelles ?",
    de: "Funktioniert es mit echten Daten?",
    pt: "Funciona com dados reais?",
    ja: "実データでは通用するのか",
    ar: "هل ينجح مع بيانات حقيقية؟",
  },
  text: {
    en: `We tested it on data we did not make. The IEEE-CIS fraud dataset: 590,540 real card transactions, of which 524,834 carry the card and address fields this needs, at a 2.46 percent fraud rate. It failed. At the threshold calibrated on our own corpus it flagged almost every candidate it produced, a lift of 1.04, barely better than chance. Ranking by score does better, but not by much: the top tenth reaches 1.12, and the top 2 percent scores 0.36, worse than the base rate. The reason is structural. One card in that data carries 14,112 transactions and the largest cluster runs to 1,568 accounts, so attribute reuse saturates and stops separating anything. The threshold does not transfer, and the ranking barely does. An earlier run on a 20,000 row slice showed 1.62, and that was published before it was checked against the full dataset. It was a small sample artefact. Both numbers are in the repository with the code that produced them, because a project that only publishes the measurement that flattered it has not measured anything.`,
    hi: `हमने इसे उस डेटा पर परखा जो हमने नहीं बनाया। IEEE-CIS फ़्रॉड डेटासेट: 590,540 असली कार्ड ट्रांज़ैक्शन, जिनमें से 524,834 में वे कार्ड और पते के फ़ील्ड हैं जो इसे चाहिए, और फ़्रॉड दर 2.46 प्रतिशत। यह विफल रहा। अपने ख़ुद के डेटा पर तय की गई थ्रेशोल्ड पर इसने लगभग हर संभावित क्लस्टर को फ़्लैग कर दिया — लिफ़्ट सिर्फ़ 1.04, यानी अंदाज़े से मुश्किल से बेहतर। स्कोर से क्रम लगाना थोड़ा बेहतर है, पर बहुत नहीं: शीर्ष दस प्रतिशत पर 1.12, और शीर्ष दो प्रतिशत पर 0.36 — जो आधार दर से भी ख़राब है। कारण संरचनात्मक है। उस डेटा में एक कार्ड पर 14,112 ट्रांज़ैक्शन हैं, और सबसे बड़ा क्लस्टर 1,568 अकाउंट तक जाता है, इसलिए एट्रिब्यूट रीयूज़ संतृप्त हो जाता है और कुछ भी अलग नहीं कर पाता। थ्रेशोल्ड दूसरे डेटा पर नहीं चलती; रैंकिंग बमुश्किल चलती है। पहले 20,000 पंक्तियों के एक टुकड़े पर 1.62 मिला था, और वह पूरे डेटासेट से जाँचे बिना प्रकाशित हो गया। वह छोटे नमूने की ग़लती थी। दोनों आँकड़े उन्हें बनाने वाले कोड समेत रिपॉज़िटरी में हैं, क्योंकि जो प्रोजेक्ट सिर्फ़ अपनी तारीफ़ वाला माप छापता है, उसने असल में कुछ मापा ही नहीं।`,
    es: `Lo probamos con datos que no creamos nosotros. El conjunto IEEE-CIS de detección de fraude: 590.540 pagos reales con tarjeta, de los cuales 524.834 tienen los campos de tarjeta y dirección que esto necesita, con una tasa de fraude del 2,46 por ciento. Falló. Con el umbral calibrado sobre nuestro propio corpus marcó casi todos los candidatos que generó: un lift de 1,04, apenas mejor que el azar. Ordenar por puntuación va mejor, pero no mucho: el decil superior llega a 1,12 y el 2 por ciento superior obtiene 0,36, peor que la tasa base. La razón es estructural. Una sola tarjeta en esos datos acumula 14.112 pagos, y el grupo más grande llega a 1.568 cuentas, así que la reutilización de atributos se satura y deja de separar nada. El umbral no se transfiere, y el orden apenas. Una ejecución anterior sobre 20.000 filas dio 1,62, y se publicó antes de comprobarla contra el conjunto completo. Fue un artefacto de muestra pequeña. Ambas cifras están en el repositorio junto al código que las produjo, porque un proyecto que solo publica la medición que lo favorece no ha medido nada.`,
    fr: `Nous l'avons testé sur des données que nous n'avons pas créées. Le jeu IEEE-CIS de détection de fraude : 590 540 paiements par carte réels, dont 524 834 comportent les champs de carte et d'adresse nécessaires, avec un taux de fraude de 2,46 pour cent. Il a échoué. Avec le seuil calibré sur notre propre corpus, il a signalé presque tous les candidats produits : un lift de 1,04, à peine mieux que le hasard. Classer par score fait mieux, mais pas beaucoup : le dixième supérieur atteint 1,12, et les 2 pour cent supérieurs obtiennent 0,36, moins bon que le taux de base. La raison est structurelle. Une seule carte dans ces données porte 14 112 paiements, et le plus grand groupe atteint 1 568 comptes : la réutilisation d'attributs sature et ne sépare plus rien. Le seuil ne se transfère pas, et le classement à peine. Un essai antérieur sur 20 000 lignes donnait 1,62, publié avant d'être vérifié sur le jeu complet. C'était un artefact de petit échantillon. Les deux chiffres sont dans le dépôt avec le code qui les a produits, car un projet qui ne publie que la mesure qui l'arrange n'a rien mesuré.`,
    de: `Wir haben es an Daten getestet, die wir nicht selbst erzeugt haben. Der IEEE-CIS-Betrugsdatensatz: 590.540 echte Kartenzahlungen, davon 524.834 mit den benötigten Karten- und Adressfeldern, bei einer Betrugsquote von 2,46 Prozent. Es ist gescheitert. Mit dem auf unserem eigenen Datensatz kalibrierten Schwellenwert markierte es fast jeden erzeugten Kandidaten — ein Lift von 1,04, kaum besser als Zufall. Eine Sortierung nach Punktzahl schneidet besser ab, aber nicht viel: das oberste Zehntel erreicht 1,12, die obersten 2 Prozent nur 0,36, schlechter als die Grundrate. Der Grund ist struktureller Natur. Eine einzige Karte trägt dort 14.112 Zahlungen, und die größte Gruppe umfasst 1.568 Konten, sodass die Attribut-Wiederverwendung sättigt und nichts mehr trennt. Der Schwellenwert überträgt sich nicht, und die Rangfolge kaum. Ein früherer Lauf über 20.000 Zeilen ergab 1,62 und wurde veröffentlicht, bevor er am vollständigen Datensatz geprüft war. Es war ein Artefakt der kleinen Stichprobe. Beide Zahlen liegen mit dem erzeugenden Code im Repository, denn ein Projekt, das nur die schmeichelhafte Messung veröffentlicht, hat nichts gemessen.`,
    pt: `Testamos com dados que não fomos nós que criamos. O conjunto IEEE-CIS de detecção de fraude: 590.540 pagamentos reais com cartão, dos quais 524.834 têm os campos de cartão e endereço necessários, com taxa de fraude de 2,46 por cento. Falhou. Com o limiar calibrado no nosso próprio corpus, ele marcou quase todos os candidatos que gerou — um lift de 1,04, mal melhor que o acaso. Ordenar por pontuação vai melhor, mas não muito: o decil superior chega a 1,12 e os 2 por cento superiores ficam em 0,36, pior que a taxa base. A razão é estrutural. Um único cartão nesses dados carrega 14.112 pagamentos, e o maior grupo chega a 1.568 contas, então a reutilização de atributos satura e deixa de separar qualquer coisa. O limiar não se transfere, e a ordenação mal. Uma execução anterior sobre 20.000 linhas deu 1,62, e foi publicada antes de ser conferida contra o conjunto completo. Foi um artefato de amostra pequena. Os dois números estão no repositório junto ao código que os produziu, porque um projeto que só publica a medição que o favorece não mediu nada.`,
    ja: `自分たちが作っていないデータでも試しました。IEEE-CIS の不正検知データセットです。実際のカード決済が590,540件、そのうち必要なカード情報と住所の項目が揃っているのが524,834件、不正率は2.46パーセントでした。結果は失敗です。自前のデータで較正したしきい値では、生成した候補のほぼすべてを検出してしまいました。リフトは1.04、ほとんど当てずっぽうと変わりません。スコア順に並べるほうがましですが、大きな差ではありません。上位10パーセントで1.12、上位2パーセントでは0.36と、基準率を下回ります。理由は構造的なものです。このデータでは1枚のカードに14,112件の決済がぶら下がり、最大のクラスタは1,568アカウントに達します。属性の再利用が飽和し、何も切り分けられなくなるのです。しきい値は移植できず、順位付けもかろうじて残る程度でした。以前に20,000行の一部だけで試したときは1.62という数字が出、全体で確かめる前に公開してしまいました。小さな標本による見かけの結果でした。両方の数字を、それを出したコードとともにリポジトリに残しています。都合のよい測定結果だけを公開するプロジェクトは、何も測っていないのと同じだからです。`,
    ar: `اختبرناه على بيانات لم نصنعها. مجموعة IEEE-CIS لكشف الاحتيال: 590,540 عملية دفع حقيقية بالبطاقة، منها 524,834 تحمل حقول البطاقة والعنوان التي يحتاجها النظام، بمعدل احتيال 2.46 بالمئة. وقد فشل. فعند الحد الفاصل المعاير على بياناتنا نحن، وضع علامة على كل المرشحين تقريبًا الذين أنتجهم — برفع قدره 1.04، وهو بالكاد أفضل من الاختيار العشوائي. الترتيب حسب الدرجة أفضل، لكن ليس كثيرًا: العُشر الأعلى يبلغ 1.12، وأعلى 2 بالمئة يسجّل 0.36، أي أسوأ من المعدل الأساسي. والسبب بنيوي. فبطاقة واحدة في تلك البيانات تحمل 14,112 عملية، وأكبر مجموعة تصل إلى 1,568 حسابًا، فتتشبّع إعادة استخدام السمات وتتوقف عن الفصل بين أي شيء. الحد الفاصل لا ينتقل، والترتيب بالكاد ينتقل. وفي تشغيل سابق على شريحة من 20,000 صف ظهر الرقم 1.62، ونُشر قبل التحقق منه على المجموعة الكاملة. كان أثرًا لعيّنة صغيرة. الرقمان كلاهما في المستودع مع الشيفرة التي أنتجتهما، لأن المشروع الذي ينشر فقط القياس الذي يجامله لم يقِس شيئًا.`,
  },
};

export const LANDING_EXPLAINERS: Explainer[] = [WHY, FLAGGING, CALIBRATION, REAL_DATA];
