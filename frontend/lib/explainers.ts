...
const WHY: Explainer = {
  ...
  text: {
    en: `Most fraud tools score one payment at a time. That works for a stolen card, and it cannot work for a ring, because coordination does not exist inside a single payment. It exists between them. RingSentinel builds a graph of which accounts share a device, an address, or a card, and scores the cluster instead of the payment. On the seeded corpus that found 12 rings out of 12, with no false flags. Every score breaks into four named signals, so a reviewer can argue with the number rather than trust it. And nothing here can block, freeze, or decline anyone.`,
...
const FLAGGING: Explainer = {
  ...
  text: {
    en: `Four signals, each measured and weighted. Attribute reuse, at 45 percent, asks how many separate accounts funnel through one card, device, or address. Timing regularity, at 25 percent, asks whether the gaps between payments look like a person or a script. Concentration asks how much of the cluster's volume runs through the shared attribute. Account shallowness asks whether these accounts have real histories, or exist only to place one discounted order. A shared address counts for less than a shared card, deliberately. Households share addresses. A weight of 0.4 is what keeps a family from being flagged as a crew.`,
...
const PAGE: Explainer = {
  ...
  text: {
    en: `This is the review queue. Every row is a cluster of accounts the detector flagged as possibly coordinated. Never a single payment, always a group. Nothing on this page has been acted on. A flag is a request for your attention, not a decision, and no code path in this system can block, freeze, or decline anyone. Select any cluster and its full case opens below: what Claude wrote about it, the four signals behind the score, the accounts and what they share, and the decision, which is yours to make.`,
